...
var M_media{w in W, d in 1..5, s in 1..4} binary; #1 if a media meeting is placed on this week, day, shift, 0 otherwise
...
# Department constraint: child
subject to one_child_meeting_per_five_weeks:
	sum{w in 1..5}(sum{d in 1..5}(sum{s in S[d]} M_child[w,d,s])) = 1;

subject to two_child_meetings_per_ten_weeks{w in 1..5}:
	M_child[(w + 4) mod 10 + 1,1,1] = M_child[w,1,1];

subject to no_task_when_child_meeting{i in I_child, w in W, d in 1..5, s in S[d], j in {'Exp','Info','PL'}}:
	M_child[w,d,s] + x[i,w,d,s,j] <= 1;

# Department constraint: adult
subject to one_adult_meeting_per_five_weeks:
	sum{w in 1..5}(sum{d in 1..5}(sum{s in S[d]} M_adult[w,d,s])) = 1;

subject to two_adult_meetings_per_ten_weeks{w in 1..5}:
	M_adult[(w + 4) mod 10 + 1,1,1] = M_adult[w,1,1];

subject to no_task_when_adult_meeting{i in I_adult, w in W, d in 1..5, s in S[d], j in {'Exp','Info','PL'}}:
	M_adult[w,d,s] + x[i,w,d,s,j] <= 1;

# Department constraint: media
subject to one_media_meeting_per_five_weeks:
	sum{w in 1..5}(sum{d in 1..5}(sum{s in S[d]} M_media[w,d,s])) = 1;

subject to two_media_meetings_per_ten_weeks{w in 1..5}:
	M_media[(w + 4) mod 10 + 1,1,1] = M_media[w,1,1];
