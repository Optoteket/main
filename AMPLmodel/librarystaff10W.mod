...
param num_PL_week := 1; #Max num of PL in a week
param num_PL_week_10_weeks := 3; #Max num of PL in a 10 weeks
param num_shifts_per_time := 2; #Max num of shifts at the same time in a week
...
subject to max_three_shifts_at_same_hours_per_week{i in I, w in W, s in 1..3}:
	sum{d in 1..5} y[i,w,d,s] <= num_shifts_per_time;
