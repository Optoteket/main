...
set I_child; #Set of all workers that belong to the child department
set I_adult; #Set of all workers that belong to the adult department
set I_media; #Set of all workers that belong to the media department
...
subject to no_big_meeting_if_boss{i in I diff I_big_meeting, w in W}:
	sum {s in 1..3} (sum{j in {'Exp','Info','PL'}} x[i,w,1,s,j]) <= (1-M_big[w,1,1]);

######################## Department meeting constraint: child #########################
subject to one_child_meeting_per_five_weeks:
	sum{w in 1..5}(sum{d in 1..5}(sum{s in 1..3} M_child[w,d,s])) = 1;

subject to two_child_meetings_per_ten_weeks{w in 1..5, d in 1..5, s in 1..3}:
	M_child[(w + 4) mod 10 + 1,d,s] = M_child[w,d,s];

subject to no_task_when_child_meeting{i in I_child, w in W, d in 1..5, s in 1..3, j in {'Exp','Info','PL','LOW'}}:
	M_child[w,d,s] + x[i,w,d,s,j] <= 1;
...
subject to two_adult_meetings_per_ten_weeks{w in 1..5, d in 1..5, s in 1..3}:
	M_adult[(w + 4) mod 10 + 1,d,s] = M_adult[w,d,s];

subject to no_task_when_adult_meeting{i in I_adult, w in W, d in 1..5, s in 1..3, j in {'Exp','Info','PL','LOW'}}:
	M_adult[w,d,s] + x[i,w,d,s,j] <= 1;
...
subject to one_media_meeting_per_five_weeks:
	sum{w in 1..5}(sum{d in 1..5}(sum{s in 1..3} M_media[w,d,s])) = 1;

subject to two_media_meetings_per_ten_weeks{w in 1..5, d in 1..5, s in 1..3}:
	M_media[(w + 4) mod 10 + 1,d,s] = M_media[w,d,s];

subject to no_task_when_media_meeting{i in I_media, w in W, d in 1..5, s in 1..3, j in {'Exp','Info','PL','LOW'}}:
...
#extra constraint added since two lows are possible (evening and day)
...
######################### Time constraints #################################
#Allowing only three shifts at a certain time each week, not accounting Library on Wheels
subject to max_three_shifts_at_same_hours_per_week{i in I, w in W, s in 1..3}:
	sum{d in 1..5} y[i,w,d,s] <= 2;

#Worker 25 works only weekends on HB
subject to no_weekend_tasks_except_HB{w in W, d in 1..4, j in {'Exp', 'Info'}}:
	x[25,w,d,4,j] = 0;
