...
subject to help_constraint1{i in I, w in W, d in D, s in S[d], j in J[d]}:
	stand_in[i,w,d,s,j] >= sum {v in V} (h[i,v]*qualavail[i,w,d,s,j]) + (1-x[i,w,d,s,j]) - 1; #Qualified, available and not working a shift

subject to help_constraint2{i in I, w in W, d in D, s in S[d], j in J[d]}:
	stand_in[i,w,d,s,j] <= sum {v in V} (h[i,v]*qualavail[i,w,d,s,j]);
...
#	x[i,w,d,s,j] <= sum {v in V} (h[i,v]*qualavail[i,w,d,s,j]);
