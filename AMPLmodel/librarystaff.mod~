...
set J{D}; #Set of total outer tasks at a shift S
...
param task_worker_demand{J{S},S{D},D}; #number of workers required for task type j shift s on day d
param qualavail{I,J{S},S{D},D,W} binary; #Worker i qualified and available for task type j shift s day d week w
param meeting{S{D},D,W} binary;

### Variables ###
var x{i in I, j in J[s], s in S[d], d in D, w in W} binary; #1 if worker i is assigned task type j in shift s day d week w
...
### Constraints ###
subject to task_assign_amount{s in S[d], j in J[s], d in D, w in W}:
	sum{i in I} x[i,j,s,d,w] = task_worker_demand[j,s,d];

subject to single_task_per_day{i in I, d in D, w in W}:
	sum{s in S[d]}(sum {j in J[s]} x[i,j,s,d,w]) <= 1;

#subject to one_task_per_weekend{i in I, d in 6..7, w in W}:
#	sum{s in S2}(sum {j in J[s]} x[i,j,s,d,w]) <= 1;

subject to librarians_at_infodesks{i in I, s in S[d], d in D, w in W}:
	x[i,2,s,d,w] <= skill[i];

subject to find_lowest_stand_in_amount_no_weekends{s in S[d], j in J[s], d in 1..5, w in W}: #RHS: number of qualified workers at work that is available (not assigned to any task). meeting = 1 if any of the divisions has a meeting.
...
#	sum{w in W}(sum {d in 6..7}(sum {s in S[d]}(sum {j in J[s]} x[i,j,s,d,w]))) = 2; #Note: Does not mean they are contiguous!
