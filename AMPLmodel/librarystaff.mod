...
param N3 := 1; #Prioritize varied week shifts
...
#var shifts_that_differ_between_days{i in I, w in W, d in 1..5, d_prime in 1..5, s in 1..3} binary; #Shift that differs between different days for a worker a certain week
var diff_num_same_shifts{i in I, w in W, s in 1..3, s_prime in 1..3} integer;
...
maximize objective: #Maximize stand-ins and create schedules with similar weeks for each worker
	...
	# + N3*sum{i in I}(sum{w in W}(sum{d in 1..4}(sum{d_prime in (d+1)..5}(sum{s in 1..3} shifts_that_differ_between_days[i,w,d,d_prime,s]))));
