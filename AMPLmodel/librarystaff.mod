...
param N := 10; #The bigger, the more priority to maximize stand-ins
...
subject to three_days_weekends{i in I_weekend_avail, w in W, j in J[d]}:
	sum {s in S[6]} x[i,w,6,s,j] + sum {s in S[7]} x[i,w,7,s,j] = 2*h[i,w];  #+ x[i,w,5,4,j] = 3*h[i,w];
...
#	sum {j in J[5]} x[i,w,5,4,j] = 1*h[i,w];
