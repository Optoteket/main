...
void Library::create_initial_solution(){
  ...
  //1.4 Clear objective function
  weekend_objective_function.clear();

  //2. Read demand from demand file
  read_demand();

  //3. Create workers in library from worker file
  create_workers();

  //4. Find weekend workers
...
void Library::optimize_weekends(int iterations, int destroy_amount, double weights[3]){
  //Clear all obj fun vectors
  weekend_objective_function.clear();
  ...
  //INITIAL SOLUTION: Destroy and repair all weekends, until feasible initial solution is found
  bool feasible = false;
  while(!feasible){
    remove_weekday_tasks();
    destroy_weekend(35);
    repair_weekend();
    feasible = compare_avail_demand();
    if(feasible){
      feasible = place_BokB();
      feasible = compare_avail_demand();
    }
  ...
  //Find library cost
  set_wend_library_cost(weights);

  //LOOP OUTER: Solution improvement
  for (int i=0; i < iterations; i++){
	...
	for(int d = 0; d < NUM_DAYS; d++){
      ...
      destroy_weekend(destroy_amount);
      repair_weekend();
	...
	feasible = compare_avail_demand();
      ...
      if(!feasible){
	cout << "************ Infeasible solution (infeas loop)***********" << endl;
	//Save old solution
	worker_list.clear();
    ...
    //SA component: find exponential cooling
    ...
    cout << "Stand in cost: " << stand_in_cost << endl;
    cout << "Shift avail cost: " << shift_avail_cost << endl;
    cout << "Day avail cost: " << day_avail_cost << endl;
    cout << "Perm library_cost: " << library_cost << endl;
  }
  
  //Final comparison: overwrite solution if better solution is saved
  if (library_cost < library_max_cost){
  ...
  //Print statistics for the final solution
  ...
  cout <<"Library max cost (wend): " << library_max_cost << " Library current cost: " << library_cost << endl;
...
void Library::optimize_weekday_tasks(int iterations, int destroy_amount){
  //Reset variables
  ...
  for(int i=0; i<iterations; i++){
    ...
    //Remove all weekday tasks but BokB and evening if they are placed
    if(i>0){
      remove_weekday_tasks_not_BokB_or_evening();
    }
    repair_tasks("perm");

    //Show progress of task repair and destroy in terminal
...
void Library::set_wend_library_cost(double weight[3]){
  //Set min and average costs
  set_wend_stand_in_cost();
  set_wend_shift_avail_cost();
  set_wend_day_avail_cost();
 
  //Set library cost as weighted sum
  library_cost = 
    weight[0]*stand_in_cost
...
void Library::set_wend_day_avail_cost(){
...
void Library::set_wend_shift_avail_cost(){
...
void Library::set_wend_stand_in_cost(){
...
bool Library::place_BokB(){
  ...
  //Place Thursday evening task on 25 when possible
  for(int i=0; i< (int) worker_list.size(); i++){
  ...
  //Place Wednesday evening task on 14 and 37 every other week
  for(int t=0; t < (int)task_list.size(); t++){
    SingleTask* current_task = &task_list[t];
    int week = current_task->get_week();
    int day = current_task->get_day();
  ...
  //Place Monday morning task on 25 and 36 every other week
  for(int t=0; t < (int)task_list.size(); t++){
    SingleTask* current_task = &task_list[t];
    int week = current_task->get_week();
    int day = current_task->get_day();
    ...
    if(worker25-> get_current_avail(week, day, shift) == current_task->get_qualification() && 
      ...
      current_demand[week][day][shift][BokB]--;
	     ...
	     worker25->get_current_task(week,day,shift) != BokB){
      //Place
...
void Library::destroy_weekend(int destroy_amount){
...
void Library::destroy_a_weekend(TaskWorker& t_worker){
...
void Library::repair_weekend(){
  ...
  while(weekend_task_list.size() > 0){
    ...
    current_task->place_workers(&destroyed_wend_workers, "perm");
...
void Library::set_wday_stand_in_cost(){
  int local_num_avail_day_workers[NUM_POSITIONS][NUM_WEEKS][NUM_WEEKDAYS];
  int min_avail_cost = lib_weight*100 + ass_weight*100;
  int min_num_ass = 0;
  int min_num_lib = 0;
