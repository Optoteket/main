...
Cons like rotations, solution time and costs are no major issues as they can be avoided by a few smarter implementations. For instance, rotations can be improved by assigning a value when a staff member is available as stand-in on a day, and from those numbers create an even distribution of possible stand-ins and therefore increase the lowest value of stand-ins through the days. This should implicitly decrease the solution time as less iterations would be required. However, to always be able to create a pool of possible week appearances regardless of problem size can easily become a major issue. Just by adding meetings and the assignment of the library on wheels tasks to the problem would make it grow considerably.
