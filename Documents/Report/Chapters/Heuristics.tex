...
In the AMPL implementation the model given in Appendix \ref{definitions} is used. However, in order to implement the heuristics, some of the model's constraints had to be relaxed and some had to be removed due to lack of time. Figure \ref{fig:AMPL_vs_heur} illustrates this process, as some constraints which are considered to be hard in the original model are softened when the heuristics are applied. The modifications of the model for the two heuristics are described in Tables \ref{tab:weekly_task_constraints} and \ref{tab:task_constraints}. The reason for relaxing the model is to create a larger neighbourhood for the heuristic to search through. This allows it to move more freely between solutions and increases the chances of finding an optimal solution.
...
\begin{table}[!h]
\centering
\caption{Week block scheduling approach; soft, relaxed and removed constraints.}
...
\ref{constr:three_PL} & Maximum number of PL per worker and week cycle. \\ \hline
% ---------------------------------------------------------
...
\multicolumn{2}{|l|}{\cellcolor{gray!90} \textbf{Relaxed Constraints}} \\
\hline 
\rowcolor{Gray} Affected constraints & Constraint alteration \\ \hline
Several. $W = W_5$ in all constraints. & Five week scheduling instead of ten week scheduling. \\ \hline
\ref{constr:obj_fcn_shifts} & Any two weeks \textit{w} and \textit{w+5} shall be as similar as possible. \\ \hline
  ...
  \centering
	\scalebox{0.7}{ \label{flow_chart}
		\begin{tikzpicture}[node distance = 2cm, auto]
			...
			\node[block,above of= enough, node distance=7.5cm] (empty) {Assign empty blocks for all workers};
