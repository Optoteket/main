...
\begin{tikzpicture}[node distance = 2cm, scale=0.8, , every node/.style={scale=0.6}]
    %Right column
    \node [scale=0.01,node distance=0cm] (invinit) {};
    \node [block, below of=invinit, node distance=2cm] (init) {Set \\ feasibility to true};
    \node [block, below of=init] (distribute) {Try distribute all tasks};
    \node [block, below of=distribute] (workercost) {Set worker obj fun};
    \node [decision, below of=workercost, node distance=3cm] (evaluatewc) {Worker cost = 0?};
    ...
    \node [block, below of=feasible, node distance=3cm] (destamount) {Set destroy amount};
    \node [block, below of=destamount] (dr) {Destroy and repair solution};
    \node [block, below of=dr] (workercost2) {Set worker costs};
    ...
    %Left column
    \node [block, left of=savebest, node distance=4cm, minimum height=3em] (count++) {Increase iterator};
    \node [decision, above of=count++, node distance=2cm] (done) {Iteration \textgreater max?};
    \node [block, above of=done, node distance=2cm] (remove) {Remove all tasks};
    
    %Get solution
    \node [decision, below of=count++, node distance=10cm] (isbest) {Current better than best?};
    \node [block, right of=isbest, minimum height=3em, node distance=3cm] (reload2) {Reload best};
    \node [block, below of=reload2, minimum height=3em, node distance=2.5cm] (finished) {Search finished!};
    
	%Invisible nodes
    \node [right of=evaluatewc, scale=0.01,node distance=2cm] (inv) {};
    \node [right of=workercost2, scale=0.01,node distance=4cm] (inv2) {};
    \node [below of=evaluatewc, right of=evaluatewc, scale=0.01,node distance=4cm] (inv3) {};
    ...
    \path [line] (evaluatewc) -- node {yes} (evaluatetasks);    
    %ERROR
%    \path [line] (evaluatewc) -- node {no} (inv);    
    \path [line] (evaluatetasks) -- node {yes} (taskcost);
    \path [line] (evaluatetasks) -- node {no} (feasible);    
    %ERROR
%	\path [line] (inv) |- (feasible);
    \path [line] (feasible) -- node {yes} (destamount);
    \path [line] (feasible) -| node {no} (count++);
    \path [line] (destamount) -- (dr);
    \path [line] (dr) -- (workercost2);
    %ERROR
%    \path [line] (workercost2) -- (inv2);
%    \path [line] (inv2) -- (inv3);
%    \path [line] (inv3) -- (evaluatewc);
    \path [line] (count++) -- (done);
    \path [line] (done) -- node {no}(remove);    
    \path [line] (best) -- node {yes}(savebest);
    \path [line] (best) -- node {no}(count++);
    \path [line] (taskcost) -- (best);
    \path [line] (remove) -- (distribute);
    %ERROR
%    \path [line] (done) -| node {no} (inv4);
