...
\begin{table}[ht]
\centering
\caption{Worker demand during a week when entering weekday phase. PL is marked as one shift, but is performed during a whole day.}
...
As in the case with most libraries, the Central Library of Norrköping also has responsibilities that fall outside of its normal daily activities. One such responsibility is weekend staffing a smaller library subsidiary in Hageby, situated in a suburban area of Norrköping. It is decided that only librarians are qualified for this task, as it implies some tasks that need certain knowledge. Also, some librarians prefer to work only at HB while others never work at HB. 
...
The Central Library of Norrköping currently has 39 library workers, 23 of which are librarians and 16 of which are assistants. All staff have different availability for performing tasks, depending on their working hours and the amount of inner work they are in charge of. In the standard case, each worker is assigned one evening per week and once per five weeks he/she is assigned to work during the weekend. The week after is normally compensated with two extra free days, placed according to the wishes of the worker.
...
When we first were presented with the scheduling task, it was explained to us that the main challenge for the library is to find a good number stand ins for all days. When we studied their schedule, we observed that only one stand in was present at the worst day. Thus, the main objective of this thesis work is to create a feasible schedule, according to the constraints explained in this section, with as many stand ins as possible at the worst day.

In this context, a stand in is defined by the as a worker who is available for outer tasks during the first three shifts, and who is not scheduled for any shift that day. Both librarians and assistants can be scheduled as a stand-in, but only librarians are qualified to take an Info task. Since the regular hour library activity are most crucial, there is no need for stand ins during evening and weekends. Similarly, there are no assigned stand ins for the Library on Wheels or for Hageby.

Apart from maximizing the number of stand-ins for each day at the library, it is desirable for the sake of the personnel to create schedules in which are repeated according to a five week pattern. Thus, the ten week schedule should become two five week schedules. 
