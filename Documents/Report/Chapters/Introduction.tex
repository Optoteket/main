...
The studied problem involves creating a ten week task distribution for all workers with a high number of stand ins at all days. We were given the workers' individual availability schedules, the staffing demand at different tasks, shifts, days and weeks as well as the constraints and demands discussed in this section. 
...
Working during a weekend implies working both Saturday and Sunday, sometimes even Friday evenings as well. When not working at HB, the Friday evening shift is also part of the weekend work. The weekend work is compensated with free days, often placed at the week following upon weekend work, in this thesis referred to as "weekrest week".


Since the number of visitors at the library varies throughout the day and also during different days of the week, so does the demand for personnel for the outer tasks. This demand is illustrated in Table \ref{tab:Outer_Task_Demand}, which is constructed according to values given by the library. 
...
Considering again the sample worker's schedule in Table \ref{tab:Bob_avail}, the worker is available throughout all days. However, four basic constraints regulate how tasks can be placed during availability. Firstly, workers are only allowed to take a maximum of one task per day. This guarantees that workers also have time for inner tasks. Secondly, workers are only allowed to perform a maximum of four tasks per week, in order for them to have a completely free day. Thirdly, workers can only be assigned a PL at most once a week. The restriction for ten weeks varies between different workers, but should never exceed four. Lastly, a worker should not have the same shift more than twice a week. This guarantees that there is fairness in the schedule so that no worker has to work too many times at a shift considered "bad" (mornings or evenings depending on individual preferences). An example of a feasible week schedule for the sample worker is provided in Table \ref{tab:Lib_feas_sched}.

 Availability for all workers is given by the library. Some workers work differently even and odd weeks, some never work evenings and some work only or never weekends. This data was used in order to solve the problem.
...
When the scheduling task first were presented, it was explained that the main challenge for the library is to find a good number stand-ins for all days. When we studied their schedule, we observed that only one stand-in was present at the worst day. Thus, the main objective of this thesis work is to create a feasible schedule, according to the constraints explained in this section, with as many stand-ins as possible at the day with the least number of stand-ins.

In this context, a stand-in is defined as a worker who is available for outer tasks during the first three shifts and who is not scheduled for any shift that day. Both librarians and assistants can be considered a stand-in, but only librarians are qualified to be assigned any task. Since the regular hour library activities are most crucial, there is no need for stand-ins during evenings and weekends. Similarly, there are no assigned stand-ins for the Library on Wheels or for Hageby.
...
\section{Method}
The method for solving the problem described above is to create a mathematical model for the problem, which will be solved using three different methods. The first method involves modeling the problem in the AMPL programming language and solving it in the commercial optimization solver CPLEX. The second and third method involves creating independent solvers in C++ and solving it using heuristics. 

Two different heuristics were to be tested. The first heuristic distributes week blocks to workers. All blocks are pre-generated and feasible according to the constraints described in the previous section. In the second heuristic, individual tasks are distributed to the workers. In order to find better solutions, and moving through the solution space, both heuristics were to be based on a Large Neighbourhood Search (LNS) algorithm.
