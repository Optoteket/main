...
\section{Other similar problems. (Fixed/flexible job scheduling problem (FJSP))}\label{FJSP}

Variations of the task assignment problem relevant for our problem include the fixed job schedule problem and the flexible job scheduling problem. The fixed job schedule problem (FJSP) has been studied since the 1970s in the context of task assignment in processors. The problem concerns the distribution of tasks with fixed starting and ending times over a workforce with identical skills, such as processing units. Krishnamoorthy\cite{}. Such problmes have been solved by I. Gertsbakh, H.I. Stern "Minimal resources for fixed and variable job schedules" and Fischetti et al. "Approximation algorithms for fixed job schedule problems" and resolved by Gupta et al. "An optimal solution for the channel assignment problem".

TFISP
The task assignment problem can be varied
...
"If all personnel/machines are identical and all tasks require the same type of machines (or if all workers have the same skills and all tasks require the same skills from the workers) and if we were to minimise the number of machines that are used, the resulting problem is a fixed job schedule problem (FJSP)." 
...
"Arkin and Silverberg [10] provide an O(n2log n) algorithm"
