...
\end{equation}
$demand_{wdsj}$ is the number of workers required week \textit{w}, day \textit{d}, shift \textit{s} for a task \textit{j}.
...
It is of interest to implement a constraint preventing workers to be assigned to HB more than once every ten weeks to avoid unfairness. The constraint can be modeled as:
\begin{equation} \label{constr:max_one_hb}
\sum_{w \in W}\sum_{d = 6}^{7}x_{iwd1HB} \leq 2, \; \forall i \in I
\end{equation}
The combination of Equations \ref{constr:max_one_hb} and \ref{constr:same_tasks} ensure that once a worker is assigned to HB it will be for two consecutive weekend days, which is as many days the previous equation allow a worker every ten weeks. Why this is preferable is described in Section \ref{section:library_tasks}.
...
\end{equation}
$l_{iwd}$ and $a_{iwd}$ are binary variables stating, for a day $d$, whether a worker is a stand-in or not. $l_{min}$ and $a_{min}$ are being maximized in the objective function; therefore, they will assume the lowest value of stand-in librarians and assistants respectively, for any day during the ten weeks. Just as with previous equations, help constraints have been left out for simplicity reasons. In this case regarding the determination of $l_{iwd}$ and $a_{iwd}$.

As stated in Section \ref{section:obj}, two weeks with a five-week interval should be as similar as possible. Equation \ref{constr:obj_fcn_shifts} aids in providing this preference:
...
\end{equation}
$y_{iwds}$ is a decision variable decided by:
...
\end{equation}
The decision variable $d_{iwds}$ in Equation \ref{constr:obj_fcn_shifts} states if there is a difference in assignment between week $w$ and $w+5$. Thus, a variable minimized in the objective function.

Another preference is to only allow one PL per week and a maximum of three PL per ten weeks. These are easily modeled with the following constraints:
...
A third preference is to have various start times of the assigned tasks each week. The following equation was implemented to meet such requirements:
