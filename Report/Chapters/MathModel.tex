...
maximize \hspace{0.3cm} L \cdot l_{min} + A \cdot a_{min} - N \cdot \sum_{i \in I}\sum_{w = 1}^{5}\sum_{d = 1}^{5}\sum_{s = 1}^{3} d_{iwds}
\end{equation}
...
\end{equation}
The duration of a PL, which is for an entire day, is the cause of these preferences. Some workers are required to have some alloted time to perform inner tasks.
